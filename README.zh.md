...
- 🚀 **并发下载**：完全重构了依赖获取逻辑，针对 51 个 Debian 包采用并发后台任务下载模式。下载时间从 ~5-10 分钟大幅缩减至 2 分钟以内。
- 🧠 **动态版本解析**：不再硬编码 Debian 软件包版本。脚本会自动通过正则匹配从 Debian 镜像站拉取最新的 `arm64` 软件包。
- 🛡️ **智能镜像降级**：默认优先使用 `mirrors.ustc.edu.cn` 中科大源，当特定架构包缺失或解析超时时，能够以包为粒度智能回退到官方 `deb.debian.org` 继续下载。
- ⏱️ **强健的网络容错**：为网页解析和包下载分别配置了 `--retry 3` 重试及 `--max-time` 自动超时机制，从容应对代理断流或连接挂死。
